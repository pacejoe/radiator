...
function addEventListeners() {
    restoreOptions();
    if (localStorage["radiatorAdvSettings"]) restoreAdvancedOptions();
...
function saveBaseOptions(callback) {
    ...
    localStorage["radiatorSettings"] = JSON.stringify(appSettings);
    callback();
}
...
function restoreOptions() {
    var appSettings = {};
    if (localStorage["radiatorSettings"]) appSettings = JSON.parse(localStorage["radiatorSettings"]);
...
function saveAdvancedOptions(callback) {
    ...
    localStorage["radiatorAdvSettings"] = JSON.stringify(advUrlObjectArray);
    bg.updateSettings();
    status.innerHTML = "OPTIONS SAVED";
    setTimeout(function () {
        status.innerHTML = "";
...
function restoreAdvancedOptions() {
    var settings = JSON.parse(localStorage["radiatorAdvSettings"]);
...
function compareSavedAndCurrentUrls(callback) {
    ...
    JSON.parse(localStorage["radiatorAdvSettings"]).forEach(function (save) {
