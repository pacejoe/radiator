...
// **** Badge Status ****
// If the window has radiator tabs enabled, make sure the badge text reflects that.
...
function checkForAndMigrateOldSettings(callback){
	if(localStorage["radiatorSettings"]) callback();
	else {
		...
		if(JSON.stringify(tempSettings) != "{}"){
			localStorage["radiatorSettings"] = JSON.stringify(tempSettings);
		}
		callback();
	}
}
...
function createBaseSettingsIfTheyDontExist(){
	if(!localStorage["radiatorSettings"]){
		settings.seconds = 15;
		settings.reload = false;
		settings.inactive = false;
		settings.autoStart = false;
		localStorage["radiatorSettings"] = JSON.stringify(settings);
	} else {
		settings = JSON.parse(localStorage["radiatorSettings"]);
	};
	if(localStorage["radiatorAdvSettings"]){
		advSettings = JSON.parse(localStorage["radiatorAdvSettings"]);
	}
	return true;
}
...
//If a user changes settings this will update them on the fly.  Called from options.js
